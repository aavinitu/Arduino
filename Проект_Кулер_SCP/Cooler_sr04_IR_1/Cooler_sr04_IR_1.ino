...
void loop() {
  ...
  Serial.print("Расстояние до объекта ИК: ");
  Serial.println(distance); //Print the value to the serial monitor

//***************вывод в интернет ИК

String data_IR = "GET /dweet/for/" + name + "?";
  data_IR += "DISTANCE_IR=" + String(distance) + " HTTP/1.1\r\n";
  data_IR += "Host: dweetpro.io\r\n\r\n";
  ...
  if (wifi.createTCP("www.dweetpro.io", 80)) {
    ...
    wifi.send(data_IR.c_str(), data_IR.length());
    wifi.releaseTCP();
  } else {
    Serial.println("IR_create TCP error");
  }
...
//**************  
  ...
  Serial.print("Расстояние до объекта УЗ: ");
  Serial.print(cm);
  Serial.println(" см.");
  ...
  data += "DISTANCE_UZ=" + String(cm) + " HTTP/1.1\r\n";
  ...
  if (wifi.createTCP("www.dweetpro.io", 80)) {
    ...
    Serial.println("UZ_create TCP error");
  }
  //delay(200);
}
